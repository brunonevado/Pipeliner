...
void toob_bam_vcf_samtools::write (std::vector <std::string> & cmds, std::vector <int> & indent, paths bucket_paths, std::vector <std::string> & err  ){
  ...
  ss << bucket_paths.get_path_samtools() << " depth -q $BASEQ -Q $MAPQ $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.bam \\";
  cmds.push_back(ss.str()); indent.push_back(1); ss.str("");
  ...
  cmds.push_back( "  > $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.conf_invar.sites" );indent.push_back(1);
  ...
  cmds.push_back( "cat $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.conf_invar.sites >>  $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.sites" );indent.push_back(1);
  ...
  if(mSNPcall){
      cmds.push_back("unset ALL_mSITES_FILES"); indent.push_back(0);
      ...
      cmds.push_back(ss.str()); indent.push_back(1); ss.str("");
      ss << "  | " << bucket_paths.get_path_vcfutils() << " varFilter -d 1 -Q $MINRMS \\";
      cmds.push_back(ss.str()); indent.push_back(1); ss.str("");
      cmds.push_back("  > $RUNFOLDER/$RUNPREFIX.mSNPcall.replicate$REP.Ind$IND.mSNPcall.flt.vcf");indent.push_back(1);

      cmds.push_back( "cut -f 2 $RUNFOLDER/$RUNPREFIX.mSNPcall.replicate$REP.Ind$IND.mSNPcall.flt.vcf \\" );indent.push_back(1);
      cmds.push_back( "  | grep -v \\# - | grep '[0-9]' - > $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.var.sites" );indent.push_back(1);
      cmds.push_back( "awk 'NR==FNR{a[$0];next} $0 in a' \\" );indent.push_back(1);
      cmds.push_back( "  $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.depth.sites \\" );indent.push_back(1);
      cmds.push_back( "  $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.var.sites \\" );indent.push_back(1);
      cmds.push_back( "  > $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.sites" );indent.push_back(1);
      cmds.push_back( "cat $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.conf_invar.sites >>  $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.sites" );indent.push_back(1);
      cmds.push_back("sort -n -u $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.sites \\"); indent.push_back(1);
      cmds.push_back("  -o $RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.sites"); indent.push_back(1);
      cmds.push_back("ALL_mSITES_FILES+=(\"$RUNFOLDER/$RUNPREFIX.replicate$REP.Ind$IND.mSNPcall.sites\")"); indent.push_back(1);

      cmds.push_back("ALL_MS_VCF_FILES+=(\"$RUNFOLDER/$RUNPREFIX.mSNPcall.replicate$REP.Ind$IND.mSNPcall.flt.vcf\")"); indent.push_back(1);
      cmds.push_back("done");indent.push_back(1);
    }
